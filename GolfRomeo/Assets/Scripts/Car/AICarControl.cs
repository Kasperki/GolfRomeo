...
public class AICarControl : MonoBehaviour
{
    ...
    [SerializeField]
    private float cautiousMaxDistance = 0.4f;                              // distance at which distance-based cautiousness begins
    [SerializeField]
    private float cautiousAngularVelocityFactor = 15f;                     // how cautious the AI should be when considering its own current angular velocity (i.e. easing off acceleration if spinning!)
    [SerializeField]
    private float steerSensitivity = 0.05f;                                // how sensitively the AI uses steering input to turn to the desired direction
    [SerializeField]
    private float accelSensitivity = 2.5f;                                // How sensitively the AI uses the accelerator to reach the current desired speed
    [SerializeField]
    private float brakeSensitivity = 0.6f;                                   // How sensitively the AI uses the brake to reach the current desired speed
    [SerializeField]
